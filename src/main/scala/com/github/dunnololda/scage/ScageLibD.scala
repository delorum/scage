...
object ScageLibD extends ScageMessageTrait with ScageXMLTrait with RendererLibD with LWJGLKeyboard with ScageColorTrait with ScageIdTrait with EventsTrait {
  ...
  def areLinesIntersect(p: DVec, p2: DVec, q: DVec, q2: DVec): Boolean = {
    ...
    /*val common = (p2.x - p.x) * (q2.y - q.y) - (p2.y - p.y) * (q2.x - q.x)
    common != 0 && {
      val rH = (p.y - q.y) * (q2.x - q.x) - (p.x - q.x) * (q2.y - q.y)
      val sH = (p.y - q.y) * (p2.x - p.x) - (p.x - q.x) * (p2.y - p.y)
  ...
  type ScageAppletD = com.github.dunnololda.scage.ScageAppletD
