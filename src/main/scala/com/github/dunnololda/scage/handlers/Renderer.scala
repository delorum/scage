...
trait RendererLib {
  ...
  def openglMove(vec: Vec) {
    GL11.glTranslatef(vec.x, vec.y, 0)
  }

  def openglMove(x: Float, y: Float) {
    GL11.glTranslatef(x, y, 0)
  }

  def openglRotate(ang_deg: Float) {
    GL11.glRotatef(ang_deg, 0, 0, 1)
  }

  def openglRotateDeg(ang_deg: Float) {
    GL11.glRotatef(ang_deg, 0, 0, 1)
  }

  def openglRotateRad(ang_rad: Float) {
    GL11.glRotatef((ang_rad * 180f / math.Pi).toFloat, 0, 0, 1)
  }

  def openglScale(scale_factor: Float) {
    GL11.glScalef(scale_factor, scale_factor, 1)
  }
...
trait Renderer extends Scage with ScageController {
  ...
  def globalScale_=(new_global_scale: Float) {
  ...
  def center_=(coord: => Vec) {
    central_coord = () => coord
  }

  private var rotation_point: () => Vec = () => Vec.zero
  private var rotation_angle: () => Float = () => 0f
